...
uint8_t *oz_aes::encrypt_CBC(uint8_t *input, unsigned int input_length, String key, unsigned int &out_length)
{
    return encrypt_CBC(input, input_length, (uint8_t *)key.c_str(), key.length(), out_length);
}
